...
class Ui_MainWindow(object):
    ...
    def recv_handler(self, stop_event):
        ...
        while not stop_event.isSet():
            ...
            if msg.startswith('0x62'):
                dtc_id = msg[4:6]
                if dtc_id == '01':
                    self.set_dtc1_state(msg)
                elif dtc_id == '02':
                    self.set_dtc2_state(msg)
                elif dtc_id == '03':
                    self.set_dtc3_state(msg)
                elif dtc_id == '04':
                    self.set_dtc4_state(msg)
            elif msg.startswith('0x6E'):
                led_id = msg[4:6]
                if led_id == '00':
                    self.set_led0_label(msg)
                elif led_id == '01':
                    self.set_led1_label(msg)
                elif led_id == '02':
                    self.set_led2_label(msg)
                elif led_id == '03':
                    self.set_led3_label(msg)
    ...
    def diag(self):
        global client
        client.sendall('0x3E01'.encode())
        self.diag_mode_on = True
    ...
    def set_led0_flags(self):
        global led0_flag, client
        if not self.diag_mode_on:
            return
        led0_flag = not led0_flag
        state = '1' if led0_flag else '0'
        msg = '0x2E00' + state
        client.sendall(msg.encode())
    ...
    def set_led1_flags(self):
        global led1_flag, client
        if not self.diag_mode_on:
            return
        led1_flag = not led1_flag
        state = '1' if led1_flag else '0'
        msg = '0x2E01' + state
        client.sendall(msg.encode())
    ...
    def set_led2_flags(self):
        global led2_flag, client
        if not self.diag_mode_on:
            return
        led2_flag = not led2_flag
        state = '1' if led2_flag else '0'
        msg = '0x2E02' + state
        client.sendall(msg.encode())
    ...
    def set_led3_flags(self):
        global led3_flag, client
        if not self.diag_mode_on:
            return
        led3_flag = not led3_flag
        state = '1' if led3_flag else '0'
        msg = '0x2E03' + state
        client.sendall(msg.encode())
